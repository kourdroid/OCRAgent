...
def _match_score_precomputed(left_str: str, left_tokens: set[str], right_str: str, right_tokens: set[str]) -> float:
...
def _find_best_match_precomputed(
    description: str,
    precomputed_candidates: list[tuple[dict[str, Any], str, set[str]]],
) -> dict[str, Any] | None:
    ...
    for candidate, norm_cand, cand_tokens in precomputed_candidates:
        score = _match_score_precomputed(
            norm_desc, desc_tokens,
            norm_cand, cand_tokens,
        )
        if score > best_score:
            best_score = score
...
def execute_3_way_match(
    ...
    # ⚡ Bolt Optimization:
    # Precompute normalized strings and token sets outside the main O(N*M) loop.
    # This prevents redundant string allocations and tokenization for each invoice line item.
    # Performance impact: Execution time for matching reduces significantly from ~O(N*M*tokens) to ~O(M) set overlaps.
    precomputed_po = []
    for po in po_lines:
        norm = _normalize_description(po.get("item_description"))
        precomputed_po.append((po, norm, set(norm.split())))

    precomputed_receipt = []
    for r in receipt_lines:
        norm = _normalize_description(r.get("item_description"))
        precomputed_receipt.append((r, norm, set(norm.split())))
    ...
    for inv_item in invoice_items:
        ...
        po_line = _find_best_match_precomputed(desc, precomputed_po)
        receipt_line = _find_best_match_precomputed(desc, precomputed_receipt)
