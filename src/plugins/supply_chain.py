...
def _prepare_candidates(candidates: list[dict[str, Any]]) -> list[tuple[dict[str, Any], str, set[str]]]:
    """Precompute normalized descriptions and token sets for candidates to avoid repeated O(N*M) string work."""
    prepared = []
    for cand in candidates:
        norm_desc = _normalize_description(cand.get("item_description"))
        tokens = set(norm_desc.split())
        prepared.append((cand, norm_desc, tokens))
    return prepared


def _match_score(left_desc: str, left_tokens: set[str], right_desc: str, right_tokens: set[str]) -> float:
    if not left_desc or not right_desc:
        return 0.0
    if left_desc == right_desc:
        return 1.0
    if left_desc in right_desc or right_desc in left_desc:
        shorter = min(len(left_desc), len(right_desc))
        longer = max(len(left_desc), len(right_desc))
...
def _find_best_match(
    ...
    prepared_candidates: list[tuple[dict[str, Any], str, set[str]]]
) -> dict[str, Any] | None:
    best_match: dict[str, Any] | None = None
    best_score = 0.0

    for candidate, norm_cand_desc, cand_tokens in prepared_candidates:
        score = _match_score(
...
def execute_3_way_match(
    ...
    # ⚡ Bolt Optimization:
    # Pre-calculate normalized descriptions and token sets for PO lines and receipt lines
    # outside the loop. This changes O(N*M) string/regex operations to O(N + M).
    prepared_po_lines = _prepare_candidates(po_lines)
    prepared_receipt_lines = _prepare_candidates(receipt_lines)
    ...
    for inv_item in invoice_items:
        ...
        po_line = _find_best_match(norm_desc, desc_tokens, prepared_po_lines)
        receipt_line = _find_best_match(norm_desc, desc_tokens, prepared_receipt_lines)
